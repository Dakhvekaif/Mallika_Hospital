...
const DoctorProfile = () => {
  ...
  // --- SAFE FALLBACKS FOR METADATA WHEN DOCTOR IS CRAWLED FRESH ---
  const pageTitle = doctor 
    ? `${doctor.name} - ${departmentName} in Jogeshwari West | Mallika Hospital`
    : "Expert Medical Specialists in Jogeshwari | Mallika Hospital";

  const pageDescription = doctor
    ? `Book an appointment with ${doctor.name}, an expert ${departmentName} at Mallika Hospital, Jogeshwari West, Mumbai.`
    : "Consult top doctors and multi-specialty medical experts at Mallika Hospital, Jogeshwari West. Check OPD schedules and book online.";

  const schemaData = doctor ? {
  ...
  // Handle errors securely without breaking index status
  ...
  return (
    <div className="min-h-screen bg-white pt-24 pb-12">
      ...
      {/* --- UNIFIED CLEAN SEO TAGS (NO INTERRUPTIONS) --- */}
      <Helmet>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          /* Inline loader keeps the underlying header layout clean for spiders */
          <div className="h-96 flex items-center justify-center text-blue-600 font-bold">
            Loading Profile...
          </div>
        ) : (
          <>
