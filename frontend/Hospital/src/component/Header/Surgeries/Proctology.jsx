...
const Proctology = () => {
  ...
  const PROCTOLOGY_SURGERY_DEPARTMENT_ID = 34; 
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        // Filter only Proctology Surgery doctors by matching the department ID 34
        const proctologySurgeryDoctors = data.filter(
          (doctor) => doctor.department === PROCTOLOGY_SURGERY_DEPARTMENT_ID
        );

        setDoctors(proctologySurgeryDoctors);
      } catch (error) {
  ...
  return (
      <div className="w-full min-h-screen bg-white pt-20">
        ...
        {/* Proctology Surgery Doctors Section */}
